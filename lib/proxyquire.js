...
// Resolves a stub relative to a module.  Note that this only resolves
  ...
  try {
    if (path[0] === '.') {
      return resolve.sync(path, {
        basedir: dirname(moduleFilename),
    ...
    // If !this._preserveCache, then we don't want to throw, since we can
    // resolve modules that don't exist.  Resolve as best we can.
    return pathResolve(dirname(moduleFilename), path)
  }
}

// This replaces a module's require function
Proxyquire.prototype._require = function (module, resolvedStubs, path) {
  assert(typeof path === 'string', 'path must be a string')
  assert(path, 'missing path')

  var resolvedPath = this._resolveRelativePath(path, module.filename)

  if (hasOwnProperty.call(resolvedStubs, resolvedPath)) {
    var stub = resolvedStubs[resolvedPath]
  ...
  if (this._containsRuntimeGlobal) {
    return this._withoutCache(module, resolvedStubs, path, Module._load.bind(Module, path, module))
  } else {
...
Proxyquire.prototype._withoutCache = function (module, stubs, path, func) {
  ...
  var resolvedStubs = {}
  Object.keys(stubs).forEach(function (stubPath) {
    var resolvedStubPath = self._resolveRelativePath(stubPath, resolvedPath)
    resolvedStubs[resolvedStubPath] = stubs[stubPath]
  })
  // Override all require extension handlers
  var restoreExtensionHandlers = this._overrideExtensionHandlers(module, resolvedStubs)
  ...
  try {
    ...
    if (this._preserveCache) {
      ...
      var ids = [resolvedPath].concat(stubIds.filter(Boolean))
