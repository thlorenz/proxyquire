...
/**
 * Restores require to its original behavior.
 * @param request The requireable module path to restore
 */
Proxyquire.prototype.restore = function (request) {
  var filename = Module._resolveFilename(request, this._parent);
  if (Module._cache[filename]) delete Module._cache[filename];
};
