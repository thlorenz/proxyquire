...
Proxyquire.prototype.load = function (request, stubs) {
  ...
  require.extensions[extname] = function ext(module, filename) {
    ...
    module.require = function (request) {
      ...
      // Only attempt to extend the dependency if we have one AND we're allowing callThru.
      if (stub && stub.hasOwnProperty('@noCallThru') ? !stub['@noCallThru'] : !self._noCallThru) {
        var actual = require_super(request);

        // If we didn't recieve a stub, use the actual dependency.
        stub = stub && fillMissingKeys(stub, actual) || actual;
      }
